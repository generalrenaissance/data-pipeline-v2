-- Domain Infra Registry - rebuild function statement timeout [2026-04-26]
--
...
-- Re-applies the function with a transaction-local timeout so callers do not
-- need to set a session workaround before invoking the rebuild.
...
begin
  ...
  -- Step 6: domain x sheet rows joined via tag_label = sheet.tag (case-insensitive,
  ...
  -- Step 7: domain x cancelled rows joined the same way.
  domain_cancelled as (
    select
      at.domain,
...
comment on function public.rebuild_domain_registry() is
  'Rebuilds infra_domain_registry from infra_accounts x infra_account_tag_mappings x infra_sheet_registry x infra_cancelled_registry. Logic mirrors src/infra/domain-registry.ts:buildDomainRegistryRows. Atomic via TRUNCATE+INSERT inside a single function call. Returns mapping_status distribution.';
